    ...
    <d2-container type="full" v-if="!show.edit&&!show.view">
      <section class="data-list-box">
        <mk-search-form :model="query" v-if="permBtn.queryButton" @search="goPage(1)">
          <el-form-item class="inline-item" prop="loginLocation">
            <el-input v-model.trim="query.params.loginLocation" @clear="goPage(1)" @keyup.native.enter="goPage(1)"
                      clearable placeholder="输入地址搜索"
                      size="mini"></el-input>
          </el-form-item>
          ...
          <el-form-item class="inline-item" prop="type">
            <el-select v-model.trim="query.params.type" @change="goPage(1)" clearable placeholder="选择项目类型搜索"
                       size="mini">
              <el-option
                label="管理端"
                :value="1">
...
import {dirUsersDelete, dirUsersReset} from "@/api/modules/sys.users.api";
