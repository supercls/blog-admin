<template>
  <section>
    <d2-container type="full" v-if="!show.edit&&!show.view">
      <section class="data-list-box">
        <mk-search-form :model="query" v-if="permBtn.queryButton" @search="goPage(1)">
          ...
          <el-form-item class="inline-item" prop="article_id">
            <el-input v-model.trim="query.params.article_id" @clear="goPage(1)" @keyup.native.enter="goPage(1)"
                      clearable placeholder="输入文章ID搜索"
                      size="mini"></el-input>
          </el-form-item>
        </mk-search-form>
        ...
        <mk-el-table @sort="sortChange" :datas="datas" :key="updataKey" :height="tableHeight"
          ...
          <template v-for="field in fields" v-if="field.show">
            ...
            <el-table-column v-else-if="field.key=='from_user_logo'" :align="field.align" :label="field.name"
                             :width="field.width" :fixed="field.fixed">
              <template slot-scope="scope">
                <el-image
                  v-if="scope.row.from_user_logo"
                  style="width: 50px; height: 50px"
                  :src="scope.row.from_user_logo"
                  :preview-src-list="[scope.row.from_user_logo]">
                </el-image>
                <span v-else>-</span>
              </template>
            </el-table-column>
            <el-table-column v-else-if="field.key=='to_user_logo'" :align="field.align" :label="field.name"
                             :width="field.width" :fixed="field.fixed">
              <template slot-scope="scope">
                <el-image
                  v-if="scope.row.to_user_logo"
                  style="width: 50px; height: 50px"
                  :src="scope.row.to_user_logo"
                  :preview-src-list="[scope.row.to_user_logo]">
                </el-image>
                <span v-else>-</span>
              </template>
            </el-table-column>
    ...
    <view-comments-info v-if="show.view" :viewData="viewData" @close="close"/>
  </section>
</template>
...
import {dirCommentsDelete, dirCommentsList} from "@/api/modules/sys.comments.api";
import GetCommentsInfo from "./get-comments-info";
import ViewCommentsInfo from "./view-comments-info";
import util from "@/libs/util";


export default {
  name: 'dir-comments-info',
  components:{ViewCommentsInfo, GetCommentsInfo,},
  data() {
    return {
      show: {
        edit: false,
      ...
      fields: [
        ...
        {key: 'article_id', name: '文章ID', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'article_title', name: '文章标题', show: true, align: "center", enableSort: false, fixed: false},
        // {key: 'pid', name: 'PID', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'from_username', name: '评论者', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'from_user_logo', name: '评论者头像', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'to_username', name: '回复对象', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'to_user_logo', name: '回复对象头像', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'content', name: '评论内容', show: true, align: "center", enableSort: false, fixed: false},
  ...
  methods: {
    getPerms() {
      ...
      if (perms.includes('*')) {
        ...
        if (perms.includes('POST /api/private/comments/create')) {
          this.permBtn.createButton = true;
        }
        if (perms.includes('POST /api/private/comments/list')) {
          this.permBtn.queryButton = true;
        }
        if (perms.includes('POST /api/private/comments/update')) {
          this.permBtn.updateButton = true;
        }
        if (perms.includes('POST /api/private/comments/delete')) {
          this.permBtn.deleteButton = true;
        }
      }
    },
    ...
    getDataList() {
      try {
        ...
        dirCommentsList(this.query).then(res => {
          let datas = res.data || []
    ...
    goDelete(data) {
      this.$toast.confirm('此操作将永久删除该条数据, 是否继续?').then(() => {
        dirCommentsDelete(data.id).then(res => {
    ...
    //添加编辑
    goEdit(data) {
      this.$toast.showSmallModal(data && data.id ? '修改' : '增加', GetCommentsInfo, {updateData: data}, data => {
        if (data) {
          this.getDataList()
        }
      })
    ...
    goView(data) {
      this.viewData=data || {}
      this.show.view=true
      // this.$toast.showSmallModal('查看', ViewCommentsInfo, {viewData: data || {}}, data => {
      //   // console.log(data)
      // })
    }
  }
