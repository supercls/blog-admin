    ...
    initChart() {
      ...
      this.chart.setOption({
        title: {
          top: 30,
          left: 'center',
          text: `${ (new Date().getFullYear() - 1) + '/' + (new Date().getMonth() + 1) + '/' + new Date().getDate() }-${ new Date().getFullYear() + '/' + (new Date().getMonth() + 1) + '/' + new Date().getDate()}博文记录`
        },
        tooltip: {
          trigger: 'item',
          formatter: function(params) {
            return params.data[1] + ' 篇博文: ' + params.data[0] ;
          }
        },
        visualMap: {
          min: 0,
